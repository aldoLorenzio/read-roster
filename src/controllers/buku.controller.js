...
const createView = catchAsync(async (req, res) => {
  const genres = await genreService.queryGenres({}, {}); // Fetch all genres without filters
  // console.log('genrees',genres);
  res.render('buku/createBuku.ejs', {
    genres,
  });
});
...
const deleteBuku = catchAsync(async (req, res) => {
  ...
  console.log('bukuService', bukuService);
  // res.status(httpStatus.OK).send({
  //   status: httpStatus.OK,
